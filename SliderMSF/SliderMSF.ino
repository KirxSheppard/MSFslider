...
/*
 * Command codes from user for Dragonframe Mode
 */
...
//Hardcoded possible speed values for live mode
#define fastSlide 2000
#define mediumSlide 1300
#define slowSlide 600

#define fastPan 500
#define mediumPan 300
#define slowPan 100

#define fastTilt 200
#define mediumTilt 120
#define slowTilt 60
...
AccelStepper stepperSlide(AccelStepper::DRIVER, 2, 3); //step, dir was 2,3
...
boolean hardStopRequested; //optional, might be removed in the future
...
void setup()
{
  ...
  while (!calibrated)
  {
    ...
    if (sign == '@') // sent from mobile app - starts from classic app mode
    {
      ifAppCtrl = true;
      initCalibration();
...
void loop()
{
  if (!ifAppCtrl) //then Dragonframe Mode
  {
    if (!nextMoveLoaded)
...
void updateMotorVelocities()
{
  ...
  // lcd.home();
  // lcd.print(motors[0].dir); //temp czy działą

  //okazuje się to nie być idealne miejsce, naley pomyśleć co z tym zrobić dalej.
  if (digitalRead(endStopRight) != HIGH && motors[0].dir)
  {
    stopMotor(0);
  }
  else if (digitalRead(endStopLeft) != HIGH && !motors[0].dir)
  {
    stopMotor(0);
  }
  nextMoveLoaded = true;
...
byte processUserMessage(char data)
{
  ...
  case MSG_STATE_CMD:
    ...
    // else if (data == '@') //just for test
    // {
    //   userCmd.command = CALIB;
    //   //ifAppCtrl = true;
    //   msgState = MSG_STATE_DONE;
    // }
  ...
  case MSG_STATE_DATA:
    if (((data >= '0' && data <= '9') || data == '-') && lastUserData == ' ')
    {
      ...
      {
        SERIAL_DEVICE.print(F("error: too many args\r\n"));
...
void processSerialCommand()
{
  ...
  for (int i = 0; i < avail; i++)
  {
    ...
    if (cmd != CMD_NONE)
    {
      ...
      if (parseError)
      {
        SERIAL_DEVICE.print(F("parse error\r\n"));
      }
    }
  }
}
...
void sendCurrentPositions()
{
  ...
  int panPos = 3;
  //stepperPan.currentPosition();
  int tiltPos = 4;
  //stepperTilt.currentPosition();
